...
lineaccesstoken = 'PS0ZKEjTXOeOeN6kM/ZTW4mjIO2nz317vOh/Axg2c7r6rTWU+DFjiI98z93HDugT+vEFYlJ5VZvsOoTOWzYYNdWdCcbyUtUcZN0wxqujko/z3yAR/INKWfMAbeRHTe1B9YyLHQriQzpyefn/eDypYAdB04t89/1O/w1cDnyilFU='
...
def event_handle(event,json_line):
    ...
    if msgType == "text":
        ...
        if msg == "สวัสดี":
            replyObj = TextSendMessage(text="จ้า สวัสดีจ้า")
            line_bot_api.reply_message(rtoken, replyObj)
        elif msg == "ทำอะไรอยู่":
            replyObj = TextSendMessage(text="นอนดูซีรีย์จ้า")
            line_bot_api.reply_message(rtoken, replyObj)
        elif msg == "ไปเที่ยวมั้ย":
            replyObj = TextSendMessage(text="ไปที่ไหนกันดีล่ะ")
            line_bot_api.reply_message(rtoken, replyObj)
        else : 
            headers = request.headers
            json_headers = ({k:v for k, v in headers.items()})
            json_headers.update({'Host':'bots.dialogflow.com'})
            url = "https://dialogflow.cloud.google.com/v1/integrations/line/webhook/ad3dbd82-619d-4666-8d16-1a188cfb3ad1"
